...
| リンク生成 | `resource_type` → URL マッピング（SCR-E01 準拠）: `workflow`→`/workflows/{resource_id}`, `project`→`/projects/{resource_id}`, `task`→`/projects`（個別タスクURLなし）, `expense`→`/expenses`（個別経費URLなし）, `null`→リンクなし |
